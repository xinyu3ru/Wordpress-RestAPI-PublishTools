...
def get_default() -> Optional[dict]:
    ...
    default_info["password"] = config.defaults().get("password")
    return default_info
...
class WordpressEndpoint:
    ...
    def read_from_config(self, host: str):
        self.host = host if host else DEFAULT_INFO['HOST']
        assert host, "no host specified and no default host found"

        self.api_host = host if host else DEFAULT_INFO['HOST']
        self.url = f"https://{self.api_host}/wp-json/wp/v2"
        self.username = DEFAULT_INFO['username']
        self.password = DEFAULT_INFO['password']
